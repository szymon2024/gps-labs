...
{- | A programm for computing the position of a GPS satellite in the
     ...
     message and GPS time provided as input. The programm is not
     intended for post‑processing applications.
     ...
     always have fresh ephemeris.

     To ensure the validity of the wrapWeekCrossover function for
     demonstration purposes, this programm uses the entryConForWrap
     function. To ensure ephemeris freshness, the programm uses the
     isEphemerisValid function, which limits the time interval. For
     demonstration purposes, this programm assumes an ephemeris
     validity interval of 4h.

     In summary, the programm does not compute time differences as
...
--   adjacent weeks.  Needs entry condition provided earlier in the
--   programm assuming that the absolute time difference is less than
--   302400 s.  In practice, this is satisfied by broadcasting
--   ephemeris every 2 hours.
wrapWeekCrossover
