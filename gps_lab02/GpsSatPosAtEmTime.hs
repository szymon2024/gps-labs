...
{- | Calculate satellite ECEF position at emission GPS time [s] from broadcast ephemeris
   | for L1/L2 pseudorange measurement (observation).
...
-- | Pseudorange for dual-frequency L1/L2 receivers
pseudorangeL1L2 :: Double -> Double -> Double
pseudorangeL1L2 c1C c2X = (f1^2*c1C - f2^2*c2X) / (f1^2 - f2^2)
...
satPosAtEmTime            
  :: Double                                  -- ^ pseudorange for L1 [m]
  -> Double                                  -- ^ pseudorange for L2 [m]
...
main = do
  let (trvw, trv) = fromCalendarGPS    2025 08 02 00 00 00.0000000      -- receiver time week number, receiver time sow
      ...
      c1C         = 23628069.060                                        -- pseudorange for L1
      c2X         = 23628076.120                                        -- pseudorange for L2
